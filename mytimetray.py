...
#                                                 ))
#                   )
    def bye(sysTrayIcon): print ("EXIT!")#thread.exit()
    menu_options1 = ()
    systray=SysTrayIcon(icons.next(), hover_text, menu_options1, on_quit=bye, default_menu_index=1)
    def setsystray(msg):
        systray.hover_text = msg
    t = thread.start_new_thread(run, ( MyArgs(action='log', dir=dir, systray= setsystray), ) )
    systray.start()